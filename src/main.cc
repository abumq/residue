...
int main(int argc, char* argv[])
{
    ...
    RLOG(WARNING) << "Your residue server is not thread-safe. Please visit https://github.com/amrayn/residue for details.";
#endif
    ...
    if (strcmp(argv[1], "--version") == 0 || (strcmp(argv[1], "-v") == 0 && argc == 2)) {
        ...
        printVersion();
        std::cout << std::endl;
        std::cout << "Please go to https://github.com/amrayn/residue/blob/master/docs/ for help" << std::endl;
        return 0;
    ...
    if (!el::Helpers::commandLineArgs()->hasParam("--force-without-root") && el::base::utils::OS::getBashOutput("whoami") != "root") {
        std::cerr << "Please run as 'root'. See https://github.com/amrayn/residue/blob/master/docs/INSTALL.md#run-as-root for more details" << std::endl;
        return 1;
    }
